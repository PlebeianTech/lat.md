...
import {
  ...
  handleResult,
  packageVersion,
} from '@plebeiantech/lat.md-core/cli';
...
program
  ...
  .action(async (query: string) => {
    ...
    const { locateCommand } =
      await import('@plebeiantech/lat.md-core/cli/locate');
...
program
  ...
  .action(async (query: string) => {
    ...
    const { sectionCommand } =
      await import('@plebeiantech/lat.md-core/cli/section');
...
program
  ...
  .action(async (options: { config?: boolean }) => {
    const { pathsCommand } =
      await import('@plebeiantech/lat.md-core/cli/paths');
    console.log(pathsCommand({ ...program.opts(), ...options, search: true }));
  });
...
external
  ...
  .action(
    async (
      ...
      const ctx = resolveContext(program.opts());
      const { externalAddCommand } =
        await import('@plebeiantech/lat.md-core/cli/external');
...
const check = program
  ...
  .action(async (opts: { fix?: boolean; profile?: boolean }) => {
    ...
    const { checkAllCommand } =
      await import('@plebeiantech/lat.md-core/cli/check');
    handleResult(
...
async function runExpand(
  ...
  const ctx = resolveContext(program.opts());
  const { expandCommand } =
    await import('@plebeiantech/lat.md-core/cli/expand');
